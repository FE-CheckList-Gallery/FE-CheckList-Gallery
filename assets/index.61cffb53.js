...
`;function V({name:e,multiple:i,fileState:o,setFileState:a}){const[n,u]=l.exports.useState(!1),c=l.exports.useRef(null),f=l.exports.useRef(0),d=h=>{h.preventDefault();let g;h.type==="drop"?g=h.dataTransfer.files:g=h.target.files,console.log(g),g.length&&(i?Object.keys(g).forEach((m,y)=>{a([...o,{id:f.current++,file:g.item(0)}])}):a([{id:0,file:g.item(0)}]))},b=h=>{h.preventDefault(),h.stopPropagation()},p=h=>{h.preventDefault(),h.stopPropagation(),u(!1)},x=h=>{h.preventDefault(),h.stopPropagation(),h.dataTransfer.files&&u(!0)},C=h=>{h.preventDefault(),h.stopPropagation(),u(!1),d(h)},$=(h,g)=>{h.preventDefault(),h.stopPropagation(),a(o.filter(m=>m.id!==g))};return t(Et,{children:s(Pt,{htmlFor:e,isDrag:n,ref:c,onDragEnter:b,onDragLeave:p,onDragOver:x,onDrop:C,children:[t("input",{onChange:d,type:"file",id:e,multiple:i}),s("div",{children:[t("h3",{children:"Drag & Drop a File Here"}),Boolean(o.length)&&o.map(h=>{var g;return s("p",{children:[(g=h.file)==null?void 0:g.name,t(It,{onClick:m=>{$(m,h.id)},icon:ue})]},h.id)})]})]})})}const Ft=r.div`
  width:100%;
  display: flex;
  flex-direction: column;
  justify-content: center;
